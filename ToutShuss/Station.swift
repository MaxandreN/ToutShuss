...
class BookStations: ObservableObject {
    ...
    func filterStations(with searchText: String) -> BookStations {
        var searchResult = stations.filter { $0.name.lowercased().contains(searchText.lowercased()) }
        var newBookStations = BookStations()
        
        newBookStations.setStations(NewListStations: searchResult)
        
        return newBookStations
    }
}
